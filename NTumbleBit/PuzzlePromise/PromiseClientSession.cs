...
namespace NTumbleBit.PuzzlePromise
{
    ...
    public class PromiseClientSession : EscrowReceiver
    {
        private abstract class HashBase
        {
            ...
            public abstract uint256 GetHash(Network network);
            public int Index
            {
                get; set;
            }
        ...
        private class RealHash : HashBase
        {
            ...
            {
                var clone = _BaseTransaction.Clone(network.Consensus.ConsensusFactory);
        ...
        public PromiseClientSession(Network network, PromiseParameters parameters, State state) : this(network, parameters)
        {
            if (state == null)
                return;
        ...
        private bool IsValidSignature(PuzzleSolution solution, HashBase hash, out ECDSASignature signature)
        {
            ...
            try
            {
                ...
                var ok = escrow.Initiator.Verify(hash.GetHash(_Network), signature);
                if (!ok)
                    signature = null;
                return ok;
            }
        ...
        internal IEnumerable<Transaction> GetSignedTransactions(PuzzleSolution solution)
        {
            ...
            for (int i = 0; i < Parameters.RealTransactionCount; i++)
            {
                ...
                if (!IsValidSignature(solution, hash, out ECDSASignature tumblerSig))
                    continue;
                var transaction = hash.GetTransaction(_Network);
