    ...
    public class RPCBlockExplorerService : IBlockExplorerService
    {
                ...
                foreach (var tx in results.ToList())
                {
                    ...
                    if (isRequester)
                    {
                        try
                        {
                            ...
                            var result = await RPCClient.SendCommandAsync("gettxoutproof", new JArray(tx.Transaction.GetHash().ToString())).ConfigureAwait(false);
        ...
        public TransactionInformation GetTransaction(uint256 txId, bool withProof)
        {
            ...
            try
            {
                ...
                var result = RPCClient.SendCommand("gettransaction", txId.ToString(), true);
                if (result == null || result.Error != null)
                {
                    //check in the txindex
                    result = RPCClient.SendCommand("getrawtransaction", txId.ToString(), 1);
                    if (result == null || result.Error != null)
                        return null;
                }
                var tx = Transaction.Parse((string)result.Result["hex"], RawFormat.Satoshi, _RPCClient.Network);
                ...
                if (withProof)
                {
                    ...
                    var result1 = RPCClient.SendCommand("gettxoutproof", new JArray(tx.GetHash().ToString()));
        ...
        {
            var result = RPCClient.SendCommand("getblock", blockId.ToString(), true);
            if (result == null || result.Error != null)
                return 0;
        ...
        public async Task<bool> TrackPrunedTransactionAsync(Transaction transaction, MerkleBlock merkleProof)
        {
            ...
            await _RPCBatch.WaitTransactionAsync(async batch =>
            {
                var result = await batch.SendCommandAsync("importprunedfunds", transaction.ToHex(), Encoders.Hex.EncodeData(merkleProof.ToBytes())).ConfigureAwait(false);
