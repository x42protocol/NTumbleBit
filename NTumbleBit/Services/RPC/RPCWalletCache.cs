    ...
    public class RPCWalletCache
    {
        ...
        private static async Task<Transaction> FetchTransactionAsync(RPCClient rpc, uint256 txId)
        {
            try
            {
                ...
                var result = await rpc.SendCommandAsync("gettransaction", txId.ToString(), true).ConfigureAwait(false);
                var tx = Transaction.Parse((string)result.Result["hex"], RawFormat.Satoshi);
                return tx;
            }
        ...
        {
            IEnumerable<Script> scripts = GetScriptsOf(entry.Transaction, _RPCClient.Network);
            lock (_TxByScriptId)
            {
        ...
        private static IEnumerable<Script> GetScriptsOf(Transaction tx, Network network)
        {
            return tx.Outputs.Select(o => o.ScriptPubKey)
                                    .Concat(tx.Inputs.Select(o => o.GetSigner(network)?.ScriptPubKey))
