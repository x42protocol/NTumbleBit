...
namespace NTumbleBit.PuzzleSolver
{
    ...
    public class SolverServerSession : EscrowReceiver
    {
        ...
        public SolverServerSession(Network network, RsaKey serverKey) : this(network, serverKey, null)
        {
        }

        public SolverServerSession(Network network, RsaKey serverKey, SolverParameters parameters)
        {
            _Network = network;
        ...
        public SolverServerSession(Network network, RsaKey serverKey, SolverParameters parameters, State state)
            : this(network, serverKey, parameters)
        {
            if (state == null)
                return;
            InternalState = state;
        ...
        public OfferInformation CheckBlindedFactors(BlindFactor[] blindFactors, FeeRate feeRate)
        {
            ...
            var virtualSize = dummy.HasWitness ? dummy.GetVirtualSize(_Network.Consensus.Options.WitnessScaleFactor) : dummy.GetSerializedSize();
            var offerTransactionFee = feeRate.GetFee(virtualSize);
        ...
        private PubKey AssertValidSignature(TransactionSignature clientSignature, Transaction offer)
        {
            ...
            var signedHash = offer.Inputs.AsIndexedInputs().First().GetSignatureHash(_Network, coin, clientSignature.SigHash);
